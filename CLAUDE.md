...
| `ephys_PAC_comodulograms.ipynb` | Session-level theta-gamma comodulogram metrics from `concatenated_lfp_stats.csv` — PAC amplitude, oscillatory event counts, and theta-gamma modulation index (all slow/fast gamma paired constructs, with the GLMM-TMB/concordance/FDR/permutation section). Split from a combined notebook on 2026-09-12, see Section 7 |
| `ephys_theta_running_metrics.ipynb` | Session-level theta metrics from `concatenated_lfp_stats.csv` — theta peak frequency and theta-running regression (slope/intercept/r/stderr). Steps 1-4 only (normality/Levene's/primary-test/box+scatter) — no paired gamma-band construct, so no GLMM section. Split off `ephys_PAC_comodulograms.ipynb` on 2026-09-12, see Section 7 |
...
- [2026-09-12] **Added Beta as a 4th GLMM family candidate, `theta_gamma_modulation_index` only**: In `ephys_PAC_comodulograms.ipynb` cell 10's shared family-selection loop (Gaussian/Gamma/Inverse-Gaussian via AIC, applied identically to all 3 paired metrics), confirmed `theta_gamma_modulation_index_{slow,fast}_gamma` is genuinely bounded (0,1) with no zeros in the actual data (range 0.0077–0.12 across 34 sessions), so Beta (`beta_family(link="logit")`, fit on raw `value`) is now included in the AIC comparison for this metric specifically, via a per-metric `beta_eligible` flag. Not extended to the other two paired metrics sharing the loop: `comodulogram_PAC_*` has exact zeros (incompatible with Beta's open (0,1) support) and `n_oscillatory_epochs_*` is a raw count (hundreds–thousands), not a proportion.
- [2026-09-12] **Correction to the entry below**: `theta_gamma_modulation_index_{slow,fast}_gamma` was moved back into `ephys_PAC_comodulograms.ipynb` rather than staying in `ephys_theta_running_metrics.ipynb` — per the user, it belongs with the comodulogram/PAC-relationship metrics, not the theta metrics. This has a structural consequence beyond just moving the metric: `theta_gamma_modulation_index` was the theta notebook's *only* paired (slow/fast gamma-band) construct, so with it gone, the entire GLMM-TMB/concordance/FDR/median-permutation-tiebreaker section (everything after Step 4) no longer has anything to operate on and was dropped from `ephys_theta_running_metrics.ipynb` entirely — that notebook is now just Steps 1-4 across 6 standalone metrics (`max_theta_freq` + the five `theta_running_*` regression outputs), no GLMM section at all. `ephys_PAC_comodulograms.ipynb` reverts to all 3 original paired constructs (`PAC`, `n_oscillatory_epochs`, `theta_gamma_modulation_index`) in its GLMM/concordance section, i.e. exactly what the pre-split notebook had.
- [2026-09-12] **Split `ephys_PAC_comodulograms.ipynb` into two notebooks by construct, dropped the LMER/RM-ANOVA sections**: `ephys_theta_running_metrics.ipynb` now holds `max_theta_freq`, the five `theta_running_*` regression outputs, and `theta_gamma_modulation_index_{slow,fast}_gamma`; `ephys_PAC_comodulograms.ipynb` keeps only `comodulogram_PAC_{slow,fast}_gamma` and `n_oscillatory_epochs_{slow,fast}_gamma`. Both notebooks carry the identical Steps 1-4 (normality/Levene's/primary-test/box+scatter), GLMM-TMB paired-metrics section (each restricted to its own paired construct — `theta_gamma_modulation_index` only in the theta notebook, `PAC`/`n_oscillatory_epochs` only in the comodulogram notebook), concordance + BH-FDR, and median-permutation tiebreaker cells; the "Two-way LMER per environment" and "Repeated measures ANOVA" sections were dropped from both per explicit request, not carried into either. The theta notebook writes to a new `LFP_analysis/theta_metrics/` subfolder (hardcoded paths in the median-permutation cells updated to match) so its outputs never collide with the comodulogram notebook's existing `LFP_analysis/` files. Motivation: Step 3's BH-FDR family (added the same session, see next entry) was pooling unrelated question-families — this split makes each notebook's own FDR family reflect one coherent construct rather than all 12 original metrics at once, though the theta notebook's own family still mixes the 5 non-independent `theta_running_*` regression outputs with 3 unrelated metrics; further splitting wasn't requested.
- [2026-09-12] **Added BH-FDR correction directly to Step 3 in the (pre-split) PAC comodulogram pipeline**: Previously Step 3 only reported raw p-values; the only FDR correction anywhere in the notebook was a separate, narrower one applied later to the GLMM-vs-secondary-test concordance subset. Added `pvalue_fdr`/`significant_fdr` columns to Step 3's own output, corrected as one family across every metric × environment test run in that cell. Checking three candidate family scopes (all-pooled, per-environment, per-metric-construct-per-environment) against the real data showed per-environment gives identical significance calls to all-pooled, and only one borderline metric (`theta_gamma_modulation_index_fast_gamma`, linear_track, raw p=0.036) flips between "ns" and significant depending on family scope — everything else is robust to this choice.
